    ...
    <main className="w-[90%] max-w-screen-xl mx-auto">
        ...
        <section className="mt-6 md:mt-8 lg:mt-10 xl:mt-12">
            ...
            <div className="flex flex-row gap-2 justify-center items-center mb-6 md:mb-8 lg:mb-10">
                {                    
                    blog.tags.split(", ").map((item, index) =>{
                        ...
                        return (
                            <p key={index} className={`px-4 py-2 rounded-full font-semibold text-base md:text-xl`} style={{backgroundColor: colorArray[number].bg, color: colorArray[number].color}}>{item}</p>
                        )
                    })
                }
            </div>
            <img 
                ...
                src={blog.coverImage ? blog.coverImage : blog.content[1] ? blog.content[1].imageFile : undefined} 
                // src={blogImg}
        ...
        <section className="flex flex-col md:flex-row gap-4 md:gap-6 lg:gap-10">
            ...
            <p className="font-semibold text-xl text-black mb-3">CONTENTS</p>
                {
                    blog.content.map((item,index)=>{
                        return (
                            <p key={index} className="text-base mb-2 text-[#475467] hover:text-[#0081f1] transition-all duration-300 ease-linear">{item.paragraphTitle}</p>
                        )
                    })
                }
            ...
            <div className="flex-1">
                ...
                <div className={`min-h-[320px] ${blog.content.length == 0 ? 'justify-center items-center flex' : ''}`}>
                    ...
                    blog.content.map((item,index)=>{
                        return (
                            <div key={index} className="mb-6 md:mb-8 lg:mb-10">
                                <p className="text-xl md:text-2xl lg:text-2xl mb-2 md:mb-4 lg:mb-6 font-semibold">{item.paragraphTitle}</p>
                                <p className="text-[#475467] text-sm md:text-base mb-2 md:mb-4 lg:mb-6">{item.paragraphText}</p>
                                {
                                    item.imageFile && <img style={{aspectRatio: "2/1.5"}} src={item.imageFile} alt="img" className="w-full mb-4" />
                                }
                            </div>
                        )
                    })
                    : <p className="text-xl md:text-2xl lg:text-3xl text-[#475467]">No content to display</p>
                }
                </div>
                    ...
                    .map((item,index)=>{
                       ...
                       return (
                            <BlogsComp 
                                ...
                                name={item.author}
                                date={item.date}
                                time={item.content[0]!== undefined ? calculateReadingTime(item.content[0].paragraphText) : calculateReadingTime("")}
